...
export default class DayView extends React.PureComponent {
	...
	setupConfigurations(props) {
		...
		let initPosition = moment().isSame(props.date, "day") ? 100 * (timeNowHour - this.props.start) + (100 * timeNowMin) / 60 : _.min(_.map(packedEvents, "top")) - this.calendarHeight / (props.end - props.start);
	...
	_renderLines() {
		...
		return range(start, end + 1).map((i, index) => {
			...
			return [
				...
				<View key={`lineHalf${i}`} style={[styles.line, { top: offset * (index + 0.5), width: width - 20 }]} />
			];
		});
	}
